...
func (cos *Cos) GetResURLFromCos(bucket, path, cosEndPoint string) string {
	return fmt.Sprintf(cosEndPoint, cos.Appid, bucket, path)
}
...
func (cos *Cos) Upload(file io.Reader, bucket, path, cosEndPoint string) (ret *Response, err error) {
	...
	bucket = strings.Trim(bucket, "/")
	path, _ = GetURLSafePath(NormPath(path))
	requestURL := cos.GetResURLFromCos(bucket, path, cosEndPoint)
	expired := cos.getExpired()
	sign := cos.SignMore(bucket, expired)
